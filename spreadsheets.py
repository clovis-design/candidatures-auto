...
def detect_headers(rows, header_keys, normalize, for_tracking=False):
    aliases = {
        ...
        'source': 'source', 'site': 'source', 'plateforme': 'source', 'sitedorigine': 'source',
        'url': 'url', 'lien': 'url', 'lienoffre': 'url', 'liendeloffre': 'url', 'urloffre': 'url',
        'datedecandidature': 'date_envoi', 'datecandidature': 'date_envoi',
        'dateenvoi': 'date_envoi', 'intituleduposte': 'stage', 'intitule': 'stage',
    }
    best = None
    for number, row in enumerate(rows[:20], 1):
        ...
        for col, value in enumerate(row, 1):
            ...
            if key and key not in mapping.values():
                mapping[col] = key
        eligible = 'coordonnees' in mapping.values() or (for_tracking and {'entreprise', 'stage'} <= set(mapping.values()))
        if eligible and (best is None or len(mapping) > len(best[1])):
            best = number, mapping
    if not best:
        if for_tracking:
            raise ValueError('En-têtes introuvables : ajoutez les colonnes Entreprise et Poste (ou Stage ciblé) dans les 20 premières lignes.')
        raise ValueError('En-têtes introuvables : ajoutez une colonne Email ou Coordonnées dans les 20 premières lignes.')
    return best


def parse_rows(rows, header_keys, normalize, company_name, for_tracking=False):
    header_row, mapping = detect_headers(rows, header_keys, normalize, for_tracking)
    ...
    for number, row in enumerate(rows[header_row:], header_row + 1):
        ...
        if len(matches) > 1 or (not for_tracking and len(matches) != 1):
            invalid.append({'row': number, 'reason': 'Un seul email est requis dans la colonne Email / Coordonnées.'})
            continue
        if for_tracking and not matches and '@' in entry.get('coordonnees', ''):
            invalid.append({'row': number, 'reason': 'Adresse email invalide dans les coordonnées.'})
            continue
        entry['email'] = matches[0].lower() if matches else ''
        entry.setdefault('stage', '')
        identity = (entry['email'], entry['stage'].casefold())
        if not for_tracking and identity in seen:
            invalid.append({'row': number, 'reason': 'Doublon email / poste ignoré.'})
            continue
        seen.add(identity)
        entry['entreprise'] = entry.get('entreprise') or (company_name(entry['email']) if entry['email'] else '')
...
def read_excel(stream, header_keys, normalize, company_name, for_tracking=False):
    ...
    try:
        ...
        return parse_rows(rows, header_keys, normalize, company_name, for_tracking)
    finally:
        wb.close()


def read_csv(stream, header_keys, normalize, company_name, for_tracking=False):
...
def update_workbook(stream, records, header_keys, normalize):
    ...
    if ws.max_row > 10000 or ws.max_column > 200:
        raise ValueError('Le tableur est limité à 10 000 lignes et 200 colonnes.')
    rows = list(ws.iter_rows(values_only=True))
    header_row, mapping = detect_headers(rows, header_keys, normalize, for_tracking=True)
    ...
    for number in range(header_row + 1, ws.max_row + 1):
        matches = EMAIL.findall(cell_text(ws.cell(number, columns['coordonnees']).value)) if 'coordonnees' in columns else []
        if len(matches) > 1:
            continue
        stage = cell_text(ws.cell(number, columns['stage']).value) if 'stage' in columns else ''
        company = cell_text(ws.cell(number, columns['entreprise']).value) if 'entreprise' in columns else ''
        if matches:
            candidates = [r for r in records if r['email'] == matches[0].lower() and
                          (not stage or r['stage'].casefold() == stage.casefold())]
        else:
            candidates = [r for r in records if company and stage and
                          r.get('entreprise', '').casefold() == company.casefold() and r['stage'].casefold() == stage.casefold()]
        if len(candidates) != 1:
